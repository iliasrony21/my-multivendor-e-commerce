...
<div class="dashboard-menu">
    <ul class="nav flex-column" role="tablist">
        ...
        <li class="nav-item">
            <a class="nav-link {{ ($route == 'user.change.password')? 'active': '' }}"  href="{{ route('user.change.password') }}" ><i class="fi-rs-marker mr-10"></i>Change Password</a>
        </li>
        <li class="nav-item">
        ...
        <form action="{{route('user.logout')}}" method="POST">
                @csrf
            <button class="dropdown-item"style="background:#ddd;" ><i class='fi fi-rs-sign-out mr-10'></i><span>Logout</span></button>
            </li>
            </form>
    </ul>
 </div>
